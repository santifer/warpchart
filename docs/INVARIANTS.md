...
| Rule | Why | Sensor |
|---|---|---|
| An absence is `null`, never `0`. | GitHub returns `edges: []` without an error for data it no longer lists; read as "grew 0" it zeroed every neighbor. | pending (phase 2) |
| A failure returns `null`, never the total. | A failed "free issues" query that falls back to the labeled count is the same lie. | pending (phase 2) |
| A sample is not a total. | "138 contributors" were the authors inside a 500-PR sample; GitHub said 213. | pending (phase 2) |
| Windows are temporal, never by count. | A window of the last N items shrinks by itself when a project speeds up, so the series jumps. | pending (phase 2) |
| Count openings, not signs. | "12 good first issues" when 2 were free: count `no:assignee -linked:pr`. | pending (phase 2) |
| A one-off correction is not a rate. | A 21.8k-star purge inside the 7-day window read as −2903★/day. | `projectCrossing` test shows the damage; purge detection test pending (phase 2) |
| No point in the future. | Stamping today at 12:00 UTC put the morning point ahead of `now`. | pending (phase 2) |
...
| UI order never comes from a `Set`. | Insertion order made the newest merger "operate" a 70K-star repo. | pending (phase 2) |
...
- **GraphQL partial errors:** aliased batches return `errors` (e.g. NOT_FOUND for one renamed repo) together with valid `data`. Throw only when `data` is null; otherwise warn and keep the rest.
...
- **A 403 with `x-ratelimit-remaining > 0` and no `retry-after` is a permission error, not quota.** Fail over to the next token instead of sleeping.
...
- A repo transfer renames the repo, never the npm package, the domain or anything outside GitHub. `mission.aliases.json` entries are **never deleted** (old keys live forever in the Blob). Anything that builds an external identifier from `owner/name` must try every name from `allNamesOf()`.
